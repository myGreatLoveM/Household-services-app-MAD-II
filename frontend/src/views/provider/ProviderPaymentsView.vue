...
import { getAllPaymentsForProviderDashboard } from '@/services/providerService'
...
const { data, isPending, refetch, isError, error } = useQuery({
  queryKey: () => ['providers', provId, 'payments', page.value],
  queryFn: () => getAllPaymentsForProviderDashboard(provId, page.value),
  ...
  <section class="mx-auto w-full max-w-7xl px-4 py-4">
    ...
    >
      <div>
        ...
        <p class="mt-1 text-sm text-gray-700">This is a list of all payments regarding bookings</p>
      </div>
    </div>
      ...
      <table class="min-w-full divide-gray-200 bg-white text-sm">
        ...
        <tbody class="divide-y divide-gray-200">
          <tr class="odd:bg-white even:bg-gray-50 text-center" v-for="(payment, i) in data.payments" :key="i" >
            ...
            <td class="whitespace-nowrap px-4 py-2 font-medium text-gray-700">{{ payment.service.toUpperCase() }}</td>
            <td class="whitespace-nowrap px-4 py-2 text-gray-700">{{ payment.booking.customer.user.profile.first_name }}</td>
            <td class="whitespace-nowrap px-4 py-2 text-gray-700">{{ payment.amount }} ₹</td>
            <td class="whitespace-nowrap px-4 py-2 text-gray-700">{{ formatDate(payment.booking.book_date) }}</td>
            <td class="whitespace-nowrap px-4 py-2 text-gray-700">
              {{ payment.status === PaymentStatus.PAID ? formatDate(payment.updated_at) : '-' }}
            </td>
            <td class="whitespace-nowrap px-4 py-2 text-gray-700">
              <span
