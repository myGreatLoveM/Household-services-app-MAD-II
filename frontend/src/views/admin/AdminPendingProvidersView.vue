...
const { data:provData, isPending:isProvDataPending, refetch: refetchPendingProviders, isError:isProvDataError, error:provDataError } = useQuery({
  queryKey: () => ["admin", "providers", page.value, UserStatus.PENDING],
  queryFn: async () => await getAllProvidersForAdminDashboard(page.value, UserStatus.PENDING),
  keepPreviousData: true,
  enabled: isEnabled.value,
});

const { data:approvedProvData, isPending:isApprovalPending, isSuccess:isApprovalSuccess, mutate:approveProvider, error:approvalError, isError:isApprovalError} = useMutation({
...
watch(approvalError, (errorVal) => {
  if (isApprovalError.value && errorVal) {
    toast.error(`Failed to approve provider ${approvedProvData.value.provUsername}!!`);
  }
})
...
watch(isApprovalSuccess, (isApprovalSuccessVal) => {
  if (isApprovalSuccessVal) {
    ...
    toast.success(`Provider ${approvedProvData.value?.provUsername} approved..`)
    refetchPendingProviders()
  }
})
    ...
    queryClient.invalidateQueries({
      predicate: () => (query) => {
        return Array.isArray(query.queryKey) &&
          query.queryKey[0] === 'admin' &&
          query.queryKey[1] === 'providers' &&
          query.queryKey[3] === UserStatus.PENDING;
      }
    })
...
const isActionButtonsDisabled = computed(() => isApprovalPending.value || isBlockPending.value || isUnblockPending.value)


const handleApproveProvider = async (provId, provUsername) => {
  approveProvider({provId, provUsername})
}
        ...
        <tr class="odd:bg-white even:bg-gray-50 text-center" v-for="(prov, i) in provData.providers" :key="i">
            ...
            <td class="whitespace-nowrap px-4 py-2 flex justify-center gap-3">
              <button
                ...
                @click="() => handleApproveProvider(prov.id, prov.user.username)"
                :class="isActionButtonsDisabled? 'bg-green-300 cursor-none': 'bg-green-600 hover:bg-green-700'"
